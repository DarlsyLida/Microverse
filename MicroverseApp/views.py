...
def add_card(request, id):
    if request.user.is_staff==True:
        ...
        if request.method == 'POST':  
            ...
            Begindatestring = str(Begindatestring)
            ...
            card= Card(bank_account=bank, card_no=cardN, expiry_date=Enddate)
            cardN=card.card_no
            card.save()        
...
def view_cards(request, id):
    if request.user.is_staff==True:
        head="View cars accounts"
        bank =  BankAccount.objects.filter(id=id)[0]
        cards = Card.objects.filter(bank_account=bank)
        return render(request, 'cards.html', context={'cards':cards, 'head':head})
    else:
         return redirect("/customers")
